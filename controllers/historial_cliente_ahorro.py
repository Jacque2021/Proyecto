...
from views.Ui_historial_cliente_ahorro import Ui_historial_cliente_a
...
class HistorialClienteAForm(QWidget, Ui_historial_cliente_a):
    def __init__(self, parent=None): #capturar instancia de mainwindows
        super().__init__(parent)
        self.parent=parent
        self.setupUi(self) 
        self.setWindowFlag(Qt.Window)#1
    ...
    def busqueda_id(self):
        ...
        data=recipes.busqueda_id_a(Id_cliente)
        if data:
    ...
    def busqueda_nombre(self):
        ...
        data = recipes.busqueda_nombre_a(nombre)
        if data:
            self.populate_table(data)
            self.pdf_historial(data)
        else:
    ...
    def busqueda_apellidos(self):
        ...
        data=recipes.busqueda_apellidos_a(apellidos)
        if data:
            self.populate_table(data)
            self.pdf_historial(data)
        else:
    ...
    def  config_table(self): #MEDIDAS DE LA TABLA
        column_labels=("ID", "NOMBRE", "APELLIDOS", "FECHA DE APERTURA", "CIERRE DE APERTURA", "CREDITOS")
    ...
    def pdf_historial(self, data):
        ...
        pdf.cell(w=0,h=5, txt="Reporte del historial del cliente por ahorro",border=0,ln=1,align='C', fill=0)
        ...
        pdf.cell(w=10,h=10, txt="ID",border=1,align='C', fill=1)
        pdf.cell(w=50,h=10, txt="NOMBRE",border=1,align='C', fill=1)
        pdf.cell(w=50,h=10, txt="APELLIDOS",border=1,align='C', fill=1)
        ...
        pdf.cell(w=20,h=10, txt="CREDITOS",border=1,align='C', fill=1, ln=1)
        data1=[]
        data1=data
        for valor in data1:
            pdf.cell(w=10,h=8, txt=str(valor[0]),border=1,align='C', fill=0)
            pdf.cell(w=50,h=8, txt=str(valor[1]),border=1,align='C', fill=0)
            pdf.cell(w=50,h=8, txt=str(valor[2]),border=1,align='C', fill=0)
            pdf.cell(w=30,h=8, txt=str(valor[3]),border=1,align='C', fill=0)
            pdf.cell(w=30,h=8, txt=str(valor[4]),border=1,align='C', fill=0)
            pdf.cell(w=20,h=8, txt=str(valor[5]),border=1,align='C', fill=0, ln=1)
        pdf.output('reporte_historial/historial_ahorro.pdf', 'F')
    def abrir(self):
        b=date.today()
        wb.open_new(f"reporte_historial\historial_ahorro.pdf")#jacque
